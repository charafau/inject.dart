...
class _InjectorBuilder {
  ...
  void _generateInjectorProviders() {
    ...
    graph.providers.forEach((provider) {
      ...
      final method = new MethodBuilder()
        ...
        ..lambda = true
        ..body =
            invokeCreateMethod(provider.injectedType, 'this').expression.code
  ...
  // is a provider.
  Expression invokeCreateMethod(InjectedType dependency, String scope) {
    if (!graph.mergedDependencies.containsKey(dependency.lookupKey)) {
      builderContext.rawLogger.warning(
        'Failed to resolve dependency on '
            '${dependency.lookupKey.toPrettyString()}',
      );
      return literalNull;
    }
  ...
  Expression _createDependencyInstantiatingExpression(
    ...
    if (dependency is DependencyProvidedByModule) {
      ...
      dependencyExpression = refer(callExpression).call(
        dependency.dependencies
            .map((d) => invokeCreateMethod(d, scope))
            .toList(),
      );
    ...
    } else if (dependency is UnprovidedDependency) {
      builderContext.rawLogger.warning(
        'No dependency found on ${dependency.lookupKey.toPrettyString()}',
      );
      dependencyExpression = literalNull;
    } else {
